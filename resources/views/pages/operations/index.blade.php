  ...
  <div id="div_table" class="relative">
    <div class="card bg-base-100 shadow-md rounded-md my-4">
      <div class="bg-gray-200 py-2 px-4 flex justify-between">
        <span>Listado de Registro de Operaciones</span>
        <a class="btn btn-sm border-none bg-conoce-green p-0 w-56" href="{{ route('operation.generate') }}">Nuevo Registro</a>
      </div>
      ...
      <div class="card-body p-2">
        ...
        <table id="table-operations" class="table table-compact w-full table-zebra">
          <thead>
            <tr>
              <th title="ID">ID</th>
              <th title="N° DE REGISTRO">N° DE REGISTRO</th>
              <th title="PERSONA QUE FÍSICAMENTE REALIZA LA OPERACIÓN">PERSONA QUE FÍSICAMENTE REALIZA LA OPERACIÓN</th>
              <th title="PERSONA EN CUYO NOMBRE SE REALIZA LA OPERACIÓN">PERSONA EN CUYO NOMBRE SE REALIZA LA OPERACIÓN</th>
...
@push('js')
  <script src="{{ asset('js/operations.js') }}" type="text/javascript"></script>

  <script>
    var __userId;
    var __operationId;

    function assign(userId, operationId) {
      __userId = userId;
    ...
    async function confirmAssign() {
      ...
      await $.ajax({
        url: `/api/v1/operation/assign`,
        type: 'post',
        contentType:'application/json',
        data: JSON.stringify({ userId: __userId, operationId: __operationId }),
        processData: false,
