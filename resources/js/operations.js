...
function operationLists() {
  _table.DataTable({
    ...
    ajax: {
      type: 'POST',
      url: `/api/v1/operation/list-datatable`,
      data: {
        userId: $('#userId').text(),
      },
    ...
    columnDefs: [
      ...
      { className: "operation-custom-width-col", targets: [2, 3] },
    ],
...
$('tbody').on('click', 'tr', function (event) {
  ...
  if (target.classList.contains('actions')) {
    ...
    if (splitted.length > 2) {
      ...
      _idSelected = id;
      $('#operationId').text(_idSelected);
      if (action === 'show') {
        window.location.href = `/operations/details/${id}`;
      }
      if (action === 'print') {
        var link = document.createElement('a');
        link.target = '_self';
        link.href = `/api/v1/operation/details/excel/${_idSelected}`;
