...
async function modify(type) {
  ...
  loading(true);
  await $.ajax({
    url: '/api/v1/scoring/modify-natural',
    type: 'PUT',
...
async function init(userId) {
  ...
  $("select[name='ocupation']").val(window.scoring.ocupationId);
  _hasCompany = window.scoring.hasCompany === 'si';
  $("select[name='hasCompany']").val(_hasCompany ? '1' : '0');
  if (_hasCompany) {
    $("select[name='aux']").hide();
    $("select[name='scstatus']").hide();
    $("select[name='ciu']").show();
  ...
  $("select[name='scstatus']").val(window.scoring.scstatusId);
  $("select[name='sensible']").val(window.scoring.sensibleId);
  _isPEP = $("select[name='sensible']").children("option:selected").text() === 'PEP';
  if (_isPEP) {
    $('#divPep').show();
