...
function distributeCommons(inpSelectName, data) {
  ...
  if (inpSelectName === 'ciu') {
    data.forEach(({ id, code, name, risk_val, weight_dni, weight_ruc }) => (opts += `<option value="${code}" data-foo="${risk_val}|${weight_ruc}|${weight_dni}">${name}</option>`));
  } else {
...
async function modify(type) {
  ...
  loading(true);
  await $.ajax({
    url: '/api/v1/scoring/modify-company',
    type: 'PUT',
...
async function init(userId) {
  ...
  console.log(window.scoring);

  $("input[name='fullname']").val(window.scoring.name);
  $("select[name='type']").val(window.scoring.typeId);
  $("select[name='size']").val(window.scoring.sizeId);
  $("select[name='obligation']").val(window.scoring.obligationId);
  $("input[name='transaction']").val(window.scoring.transaction);
  $("input[name='identification']").val(window.scoring.dni);
  $("input[name='birthday']").val(window.scoring.birthday);
  if (window.scoring.ciuId) {
    var ciu = window.scoring.ciuId.toString();
    var ciuCode = ciu.length < 5 ? `0${ciu}` : ciu;
    $("select[name='ciu']").val(ciuCode);
  }
  $("select[name='composition']").val(window.scoring.compositionId);
  _isPEP = $("select[name='composition']").children("option:selected").text() === 'PEP';
  if (_isPEP) {
    $('#divPep').show();
    $("select[name='pep']").val(window.scoring.pepId);
  } else {
    $('#divPep').hide();
  }

  $("select[name='country']").val(window.scoring.countryId);
  $("select[name='residence']").val(window.scoring.residenceId);
  $("select[name='office']").val(window.scoring.officeId);

  $("select[name='product']").val(window.scoring.productId);
  $("select[name='currency']").val(window.scoring.currencyId);
  $("select[name='funding']").val(window.scoring.fundingId);

  $("textarea[name='obs']").text(window.scoring.obs);
  _riskTotal = window.scoring.risk_val;
  var { color, risk } = setRisk(_riskTotal);
  $('#risk_total_dot').css('background-color', color);
  $('#risk_total').text(`Riesgo ${risk}: ${_riskTotal}`);

  if (window.scoring.status === 1) {
    $(`input`).each((idx, el) => $(`input[name='${el.name}']`).removeAttr('disabled'));
    $(`textarea`).each((idx, el) => $(`textarea[name='${el.name}']`).removeAttr('disabled'));
    $(`select`).each((idx, el) => $(`select[name='${el.name}']`).removeAttr('disabled'));
  }
}