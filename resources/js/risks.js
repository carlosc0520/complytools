...
async function heatRisksChart(type) {
  _loading.show();
  $("span[id^='mp']").each((i, el) => $(`#${el.id}`).text(''));
  await $.ajax({
    url: `/api/v1/risk/list/${type}/${_userId}`,
    type: 'get',
    dataType: 'json',
    success: function(res) {
      switch (type) {
        case 'inher': $('#tittexto').text('Inherentes'); break;
        case 'res': $('#tittexto').text('Residuales'); break;
        default: break;
      }
      _loading.hide();
      res.forEach(({ y, label }) => $(`#mp${label}`).text(y));
    }
  });
}
...
function riskLists() {
  _table.DataTable({
    ...
    ajax: {
      type: 'POST',
      url: `/api/v1/risk/list-datatable`,
      data: {
        userId: $('#userId').text(),
      },
    ...
    columns: [
      ...
      { data: 'colorCoord1', width: '5%' },
      { data: 'colorCoord2', width: '5%' },
    ...
    dom: "lfrti",
    columnDefs: [
      { className: "text-center", targets: [0, 2, 3, 6] }, // targets: "_all",
...
$('tbody').on('click', 'tr', function (event) {
  ...
  if (target.classList.contains('actions')) {
    ...
    if (splitted.length > 2) {
      ...
      _idSelected = id;
      if (action === 'show') {
        window.location.href = `/risks/details/${id}`;
      }
      if (action === 'print') {
        var link = document.createElement('a');
        link.target = '_blank';
        link.href = `/api/v1/risk/details/pdf/${_userId}/${_idSelected}`;
        // link.download = "pruebitas.pdf";
        link.click();
      }
      if (action === 'pass') {
        // $('#deleteUserOpen').trigger('click');
        showTeam();
      }
    }
    return;
